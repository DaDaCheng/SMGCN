...
#adapted from geom-gcn and  from https://github.com/chennnM/GCNII
...
def full_load_citation(dataset_str):
    ...
    for i in range(len(names)):
        with open("../data/cite_data/ind.{}.{}".format(dataset_str, names[i]), 'rb') as f:
    ...
    test_idx_reorder = parse_index_file(os.path.join(par_dir,"data/cite_data/ind.{}.test.index".format(dataset_str)))
...
def full_load_data(dataset_name, splits_file_path=None):
    ...
    # print(edge_index)
    # fname = dataset_name + '.txt'
    # np.savetxt(fname,edge_index,fmt='%d')
    #np.savetxt('B.csv',B,fmt='%f')
    splits_file_path=os.path.join(par_dir,'data',splits_file_path)
